...
def vote_vector_ensemble(ensemble, title, pc_thresh=0.01):
...
def seats_votes_ensemble(ensemble, title, statewide=None):
...
def mean_median_partisan_bias(ensemble, statewide=None):
