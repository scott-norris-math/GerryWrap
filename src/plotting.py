...
def get_racial_group_pretty_name(group):
    return {
        'HVAP': "Hispanic",
        'BVAP': "Black",
...
def save_racial_histograms(chamber, output_directory, group, ensemble_matrix_transposed, current_plan, comparison_plan,
                           plan_vectors, plan_legend_names, plan_colors, do_small_histogram_pics):
...
def get_racial_group_fill_color(group):
    group_fill_color = {
        'HVAP': '#2222ff',
        'BHVAP': '#33CC00',
        'BVAP': '#ff9933'
    }[group]
    return group_fill_color


def hist_ensemble_comps(chamber, ensemble_matrix_transposed, perc_thresh, title, hist_x_axis_label, fill_color,
                        do_small_histogram_pics=True, comp_plans=False, comp_plans_vv=[],
                        comp_plans_names=[], comp_plans_colors=[]):
    if do_small_histogram_pics:
...
def save_seats_voteshares_ensemble_comps_plot(chamber, output_directory, election, ensemble_matrix, current_plan,
...
def save_racial_ensemble_comps_plot(chamber, output_directory, group, ensemble_matrix, current_plan,
                                    comparison_plan, plan_vectors, plan_pnums, plan_legend_names,
                                    plan_colors):
...
def save_violin_comparison_plots(chamber, ensemble_matrix, current_plan, comparison_plan, plan_pnums, plan_legend_names,
                                 plan_colors, plan_vectors, plot_title, fill_color, h_line_label, y_axis_label):
    ...
    return smu.vote_vector_ensemble_comps(ensemble_matrix, plot_title, pc_thresh=.01, have_actual=False,
...
def save_vote_vector_ensemble_plot(chamber, output_directory, election, ensemble_matrix, plan, plan_vectors):
    plot_title = get_chamber_pretty_name(chamber) + ' District Results  (' + get_election_pretty_name(
        election) + ')'
    plot = smu.vote_vector_ensemble(np.column_stack([plan_vectors[plan], ensemble_matrix]), plot_title, pc_thresh=.01,
                                    have_actual=True)
    plot.savefig(f'{output_directory}seats-voteshares-ensemble-enacted-{chamber}-{plan}-{election}.pdf')


def save_seats_votes_ensemble_plot(chamber, output_directory, election, ensemble_matrix, plan, plan_vectors):
    plot_title = get_chamber_pretty_name(chamber) + " Seats-Votes Curve (" + get_election_pretty_name(election) + ")"
    plot = smu.seats_votes_ensemble(np.column_stack([plan_vectors[plan], ensemble_matrix]), plot_title,
                                    have_actual=True)
    plot.savefig(f'{output_directory}seats-votes-ensemble-enacted-{chamber}-{plan}-{election}.pdf')


def save_mean_median_partisan_bias_plot(chamber, output_directory, election, ensemble_matrix, plan, plan_vectors):
    # OPTIONAL: pass in statewide=statewide_dem_share
    plot = smu.mean_median_partisan_bias(np.column_stack([plan_vectors[plan], ensemble_matrix]), have_actual=True)
    plot.savefig(f'{output_directory}mean-median-partisan-bias-ensemble-enacted-{chamber}-{plan}-{election}.pdf')


def save_racial_plots(chamber, root_directory, ensemble_directory, plots_directory, current_plan, comparison_plans,
    ...
    for racial_group in get_racial_groups():
        ...
        ensemble_matrix = cm.load_ensemble_matrix(ensemble_directory, statistic_name)
...
def save_election_plots(chamber, root_directory, ensemble_directory, plots_directory, current_plan, comparison_plans,
                        plan_pnums, plan_legend_names, plan_colors):
        ...
        for comparison_plan in comparison_plans:
            ...
            if True:
                with Timer(name='partisan_metrics_hist2D'):
                    figure = smu.partisan_metrics_hist2D(ensemble_matrix_transposed[-250000:, :],
                                                         plan_vectors[comparison_plan])
...
def save_plots(chamber, root_directory, seed_description, ensemble_number, current_plan, comparison_plans):
    ...
    plots_directory = st.build_plots_directory(root_directory, ensemble_description)
    ...
    if False:
        for election in get_elections():
            ...
            statistic_name = dt.build_election_filename_prefix(election)
            ensemble_matrix_election = cm.load_ensemble_matrix(ensemble_directory, statistic_name).transpose()
            ...
            for racial_group in get_racial_groups():
                ...
                racial_group_file_prefix = dt.transform_racial_group_file_prefix(racial_group)
                statistic_name = dt.build_race_filename_prefix(racial_group_file_prefix)
                ensemble_matrix_racial = cm.load_ensemble_matrix(ensemble_directory, statistic_name).transpose()
                plan_vectors_racial = cm.load_plan_vectors(chamber, root_directory, statistic_name,
                                                           [current_plan] + comparison_plans)

                for comparison_plan in comparison_plans:
                    print(f"Plan: {comparison_plan}")
                    figure = smu.racial_vs_political_deviations(ensemble_matrix_election,
...
def draw_ensemble_map(directory: str, graph: Graph, plans: np.ndarray, ensemble_description: str,
                      plan_relative_number: int, plan_absolute_number: int):
    plots_directory = st.build_plots_directory(directory, ensemble_description)
    ut.ensure_directory_exists(plots_directory)

    draw_plan_map(graph, plans[plan_relative_number],
        ...
        if True:
            ...
            for ensemble_description in ['TXHD_2176_1']:
                ...
                for x in range(0, 7):
                    plan_number = x * 100
                    print(plan_number)
                    draw_ensemble_map(directory, graph, plans, ensemble_description, plan_number, plan_number)
                    # plt.show()
